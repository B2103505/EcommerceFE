import React, { useEffect, useState } from "react";
import { useParams } from "react-router-dom";
import { useSelector } from "react-redux";
import { Spin } from "antd";
import { getDetailPlant, getSuggestedPlants } from "../../Service/PlantService";
import DetailProductComponent from "../../components/DetailProductComponent/DetailProductComponent";
import ReviewSection from "../../components/ReviewCom/ReviewSection";
import CardComponent from "../../components/CardComponent/CardComponent";
import { WrapperProduct } from "../HomePage/Style"; // reuse styled wrapper

const DetailProductPage = () => {
    const { id } = useParams();
    const [product, setProduct] = useState(null);
    const [suggested, setSuggested] = useState([]);
    const [loading, setLoading] = useState(false);
    const userId = useSelector(state => state.user.User_Id);

    const fetchProduct = async () => {
        setLoading(true);
        try {
            const data = await getDetailPlant(id);
            setProduct(data?.data);

            // Gọi luôn API gợi ý sản phẩm
            const suggestedData = await getSuggestedPlants(id, 8);
            setSuggested(suggestedData || []);
        } catch (error) {
            console.error("Lỗi khi lấy chi tiết cây:", error);
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        if (id) fetchProduct();
    }, [id]);

    return (
        <div style={{ padding: '0 120px', background: '#efefef', minHeight: '100vh' }}>
            {loading ? (
                <Spin size="large" />
            ) : (
                product && (
                    <>
                        <DetailProductComponent data={product} />
                        <div style={{ marginTop: 32 }}>
                            <ReviewSection plantId={product._id} userId={userId} />
                        </div>

                        {/* Danh sách gợi ý sản phẩm */}
                        {suggested.length > 0 && (
                            <div style={{ marginTop: 40, backgroundColor: '#f0f8ff', padding: '20px', borderRadius: '8px' }}>
                                <h2>Các sản phẩm gợi ý</h2>
                                <WrapperProduct>
                                    {suggested.map((plant, idx) => (
                                        <CardComponent key={`${plant._id}-${idx}`} data={plant} />
                                    ))}
                                </WrapperProduct>
                            </div>
                        )}
                    </>
                )
            )}
        </div>
    );
};

export default DetailProductPage;
